...
import { FlashSaleWideboard } from "../wingo/flash_sale/FlashSaleWideboard";
...
describe("FlashSaleWideboard (994x500)", () => {
  ...
  it("renders headline, price, CTA, and disclaimer verbatim", () => {
    const html = renderToStaticMarkup(<FlashSaleWideboard {...baseProps} />);
  ...
  it("specifies the 994x500 outer dimensions", () => {
    const html = renderToStaticMarkup(<FlashSaleWideboard {...baseProps} />);
    expect(html).toMatch(/width:\s*['"]?994/);
    expect(html).toMatch(/height:\s*['"]?500/);
  });
});
