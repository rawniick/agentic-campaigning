...
import { FlashSaleSquare } from "../wingo/flash_sale/FlashSaleSquare";
...
describe("FlashSaleSquare (1200x1200 — SEA Ad Extension)", () => {
  ...
  it("renders headline, price, CTA, and disclaimer verbatim", () => {
    const html = renderToStaticMarkup(<FlashSaleSquare {...baseProps} />);
  ...
  // V1.2-Vertrag: Square rendert jetzt die kanonische Flash-Anatomie
  ...
  it("specifies the 1200x1200 outer dimensions", () => {
    const html = renderToStaticMarkup(<FlashSaleSquare {...baseProps} />);
    expect(html).toMatch(/width:\s*['"]?1200/);
    expect(html).toMatch(/height:\s*['"]?1200/);
  });
});
